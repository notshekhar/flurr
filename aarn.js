...
class aarn {
  constructor(a) {
    if(a instanceof HTMLElement){
      ...
      if(a[0] == "#" || a[0] == "."){
        this.e = document.querySelector(a)
      }else{
        this.e = document.querySelectorAll(a)
      }
    }
  }
  ...
  styl(s){
    ...
    for(let i = 0; i<keys.length; i++){
      this.e.style[keys[i]] = s[keys[i]]
    }
  }

  value(v){
    if(this.tagName == "input" || this.tagNameA == "textarea"){
      if(v){
        this.e.value = v
      }else {
        return this.e.value
  ...
  // geting or setting attribute
  attr(a, val){
    if(this.e instanceof Array){
      if(val){
        this.e.forEach(function(i){
          i.setAttribute(a, val)
        })
      }else{
        console.log("select individual element from an array")
      }
    }else{
      if(val){
        this.e.setAttribute(a, val)
      }else{
  ...
  load(url){
    this.e.innerHTML='<object type="text/html" data="'+ url +'" ></object>'
  }
  on(event, func){
